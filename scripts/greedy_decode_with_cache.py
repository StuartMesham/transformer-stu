...
def loop_body_func(state: DecodingState) -> DecodingState:
    batch = {
        "token_ids": state.sequences[jnp.arange(batch_size), state.cur_index.ravel()].reshape(batch_size, 1),
        "position_ids": state.cur_index,
        "bidirectional_attention_mask": jnp.zeros((batch_size, 1), dtype="int32"),
    }
    logits, new_vars = transformer.apply({"params": restored_params, "cache": state.cache}, batch, mutable=["cache"], eval_mode=True)
    new_cache = new_vars["cache"]

    # Add current sampled tokens to recorded sequences.
    next_tokens = logits.argmax(axis=2)
    one_hot = jax.nn.one_hot(state.cur_index.ravel() + 1, state.sequences.shape[1], dtype=state.sequences.dtype)
    new_sequences = state.sequences * (1 - one_hot) + next_tokens * (jnp.sum(state.sequences == EOS_TOKEN_ID, axis=-1) < 2).reshape(batch_size, 1) * one_hot
...
def loop_cond_func(state: DecodingState) -> jax.Array:
    return ~jnp.logical_or(jnp.any(state.cur_index >= MAX_LENGTH - 1), jnp.all(jnp.sum(state.sequences == EOS_TOKEN_ID, axis=-1, keepdims=True) >= 2))
